 ...
 * this is copy from pokersolver v2.1.2, and did some changes to it
*/
...
class Hand {
    ...
    static winners (hands) {
        ...
        let highestRank = Math.max.apply(Math, hands.map((h) => {
            return h.rank;
        }));
...
class FourOfAKindPairPlus extends Hand {
    ...
    solve () {
        ...
        if (this.cards.length >= 6) {
            let type = `${this.cards[0].toString().slice(0, -1)}'s over ${this.cards[4].toString().slice(0, -1)}'s`;
...
class ThreeOfAKindTwoPair extends Hand {
    ...
    solve () {
        ...
        if (this.cards.length >= 7) {
            let type = `${this.cards[0].toString().slice(0, -1)}'s over ${this.cards[3].toString().slice(0, -1)}'s & ${this.cards[5].value}'s`;
            this.descr = `${this.name}, ${type}`;
        }
...
class FullHouse extends Hand {
    ...
    solve () {
        ...
        if (this.cards.length >= 5) {
            let type = `${this.cards[0].toString().slice(0, -1)}'s over ${this.cards[3].toString().slice(0, -1)}'s`;
            this.descr = `${this.name}, ${type}`;
        }
...
class TwoThreeOfAKind extends Hand {
    ...
    solve () {
        ...
        if (this.cards.length >= 6) {
            let type = `${this.cards[0].toString().slice(0, -1)}'s & ${this.cards[3].toString().slice(0, -1)}'s`;
            this.descr = `${this.name}, ${type}`;
        }
...
class ThreePair extends Hand {
    ...
    solve () {
        ...
        if (this.cards.length >= 6) {
            let type = `${this.cards[0].toString().slice(0, -1)}'s & ${this.cards[2].toString().slice(0, -1)}'s & ${this.cards[4].toString().slice(0, -1)}'s`;
            this.descr = `${this.name}, ${type}`;
        }
...
class TwoPair extends Hand {
    ...
    solve () {
        ...
        if (this.cards.length >= 4) {
            ...
            let type = `${this.cards[0].toString().slice(0, -1)}'s & ${this.cards[2].toString().slice(0, -1)}'s`;
            this.descr = `${this.name}, ${type}`;
        }
...
class PaiGowPokerHelper {
    ...
    splitHouseWay () {
        ...
        let rank = this.game.handValues.length - this.baseHand.rank;
        let handValue = this.game.handValues[rank];
        ...
        if (handValue === FiveOfAKind) {
            ...
            let sfReturn;
            let altGame = new Game('paigowpokeralt');
            let altHand = Hand.solve(this.baseHand.cards, altGame);
            let altRank = altGame.handValues.length - altHand.rank;
     ...
     */
    qualifiesValid () {
        let compareHands = Hand.winners([this.hiHand, this.loHand]);
    ...
    static winners (player, banker) {
        ...
        let hiWinner = Hand.winners([player.hiHand, banker.hiHand]);
        let loWinner = Hand.winners([player.loHand, banker.loHand]);
    ...
    static setHands (hiHand, loHand) {
        ...
        let result = new PaiGowPokerHelper(fullHand);
    ...
    static solve (fullHand) {
        let result = new PaiGowPokerHelper(fullHand = fullHand || ['']);
