...
import { privateKey } from '../utils/const';

@Injectable()
export class CardsService {
  private readonly suitMap: { [key: string]: CardColor } = {
    h: 'hearts',
    d: 'diamonds',
    c: 'clubs',
    s: 'spades',
   ...
   */
  translateCardToString(color: string, number: string | number): string {
    if (number === 10) {
      return `T${color[0]}`;
    }
   ...
   * @returns Card object
   */
  translateStringToCard(str: string): CardType {
    const rank = str.slice(0, str.length - 1);
    const suit = this.suitMap[str[str.length - 1]];
